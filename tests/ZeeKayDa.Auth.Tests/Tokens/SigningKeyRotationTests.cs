...
public sealed class SigningKeyRotationTests
{
    ...
    public void SelectActiveKey_single_key_activates_immediately_regardless_of_future_ActivatesAt()
    {
        ...
        var active = SigningKeyRotation.SelectActiveKey(timeline, now: T0, tier: BootstrapExemptionTier.KeySet);
    ...
    public void SelectActiveKey_single_key_fails_closed_when_the_calling_tier_does_not_support_the_exemption_and_ActivatesAt_has_not_arrived()
    {
        ...
        // Gating on the calling provider's tier instead (KeySourceOptions always passes
        // BootstrapExemptionTier.KeySource) keeps this closed regardless of process lifetime.
        var key = Key("AAA", activatesAt: T0 + TimeSpan.FromHours(1));
        var timeline = SigningKeyRotation.BuildActivationTimeline([key]);

        var active = SigningKeyRotation.SelectActiveKey(timeline, now: T0, tier: BootstrapExemptionTier.KeySource);
    ...
    public void SelectActiveKey_flips_to_successor_once_its_ActivatesAt_arrives()
    {
        ...
        SigningKeyRotation.SelectActiveKey(timeline, now: T0, tier: BootstrapExemptionTier.KeySet)!.Value.Key.Id.Should().Be("AAA", "successor has not activated yet");
        SigningKeyRotation.SelectActiveKey(timeline, now: successorActivatesAt, tier: BootstrapExemptionTier.KeySet)!.Value.Key.Id.Should().Be("BBB", "successor's ActivatesAt has now arrived");
    }
    ...
    public void SelectIncludedKeys_includes_both_when_successor_is_not_yet_active()
    {
        ...
        var successor = Key("BBB", activatesAt: T0 + TimeSpan.FromDays(1));
        var timeline = SigningKeyRotation.BuildActivationTimeline([predecessor, successor]);
        var active = SigningKeyRotation.SelectActiveKey(timeline, now: T0, tier: BootstrapExemptionTier.KeySet)!.Value;
    ...
    public void HasTooSoonPendingActivation_false_when_soonest_pending_ActivatesAt_is_RefreshInterval_or_further_away()
    {
        ...
        hasWarning.Should().BeFalse("exactly the refresh interval away is sufficient lead time, not 'too soon'");
    }
