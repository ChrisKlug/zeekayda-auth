...
/// <summary>
/// Exercises <see cref="SourceKeySet"/> and <see cref="SourceKeySet.Create"/>: the Current-is-
/// required rule, and Previous/Next being independently optional.
/// </summary>
public sealed class SourceKeySetTests
{
    [Fact]
    public void Create_throws_ZeeKayDaConfigurationException_when_Current_is_null()
    {
        var act = () => SourceKeySet.Create(previous: null, current: null, next: null);
    ...
    [Fact]
    public void Create_succeeds_with_only_Current_configured()
    {
        var current = CreateRsaKey("current");

        var set = SourceKeySet.Create(previous: null, current, next: null);
    ...
    [Fact]
    public void Create_succeeds_with_only_Previous_and_Current_configured()
    {
        var previous = CreateRsaKey("previous");
        var current = CreateRsaKey("current");

        var set = SourceKeySet.Create(previous, current, next: null);
    ...
    [Fact]
    public void Create_succeeds_with_only_Current_and_Next_configured()
    {
        var current = CreateRsaKey("current");
        var next = CreateRsaKey("next");

        var set = SourceKeySet.Create(previous: null, current, next);
    ...
    [Fact]
    public void Create_reports_the_signing_key_first_in_Keys()
    {
        var previous = CreateRsaKey("previous");
        var current = CreateRsaKey("current");
        var next = CreateRsaKey("next");

        var set = SourceKeySet.Create(previous, current, next);
    ...
    private static SourceKey CreateRsaKey(string id)
    {
        ...
        return new SourceKey(new SourceKeyId(id), SigningAlgorithm.RS256, publicKey, DateTimeOffset.UtcNow.AddDays(90));
    }
}
