...
public interface IAuthorizationCodeStore
{
    ...
    /// </summary>
    /// <param name="code">
    /// The raw code handle generated by the authorization endpoint. The store MUST NOT persist
    /// this value directly; it MUST derive the storage key as
    /// <c>Hex(SHA-256(code))</c> before writing to the backing store.
    /// </param>
    /// <param name="entry">
    /// The entry to persist. The store is responsible for deriving its own storage key from
    /// <paramref name="code"/>; <paramref name="entry"/> itself does not contain the cleartext
    /// code handle.
    /// </param>
    ...
    /// producing <see cref="AuthorizationCodeRedemptionResult.AlreadyRedeemed"/> is guaranteed
    /// to carry the correct <see cref="AuthorizationCodeRedemptionResult.AlreadyRedeemed.FamilyId"/>
    ...
    /// <list type="bullet">
    /// <item><description>
    ///   <see cref="AuthorizationCodeRedemptionResult.Redeemed"/> — code was valid, bound to
    ///   the presenting client, and has been atomically consumed. Proceed to issue tokens.
    /// </description></item>
    /// <item><description>
    ///   <see cref="AuthorizationCodeRedemptionResult.ClientMismatch"/> — code exists and is
    ///   unredeemed but is bound to a different client. Store has NOT consumed the code.
    ///   Return <c>error=invalid_grant</c>.
    /// </description></item>
    /// <item><description>
    ///   <see cref="AuthorizationCodeRedemptionResult.AlreadyRedeemed"/> — code has already
    ///   been redeemed. Revoke the refresh token family in
    ///   <see cref="AuthorizationCodeRedemptionResult.AlreadyRedeemed.FamilyId"/> and return
    ///   <c>error=invalid_grant</c> (RFC 9700 §2.1.1).
    /// </description></item>
    /// <item><description>
    ///   <see cref="AuthorizationCodeRedemptionResult.NotFound"/> — code is unknown to the
