...
public static class ZeeKayDaAuthBuilderWindowsCertificateStoreSigningExtensions
{
    ...
    /// in <paramref name="configure"/>. With one registered certificate it is the active signer
    /// immediately; with two or more, the one whose <c>NotBefore</c> has arrived and is most recent
    /// wins. See <see cref="SigningKeyRotation"/> for the full model.
    /// </para>
    /// <para>
    /// The store is read exactly once, at startup — adding, removing, or updating a registered
    /// certificate requires a host restart to be picked up.
    /// </para>
    /// </remarks>
