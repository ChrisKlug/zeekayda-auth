...
- **A signing-key provider's failure message no longer repeats the underlying parser's text** (#764).
  ...
  threw. A failure message is a plain public-API string that reaches the operator's log through the
  host's own unhandled-startup-exception logger, which is not a sanitizing one, so neither by-key
  redaction nor exception wrapping could act on it. All five sites now name the exception type with
  `ex.GetType().FullName` and carry the original as `InnerException`, matching what the framework
  already did everywhere else. Diagnostic detail is not lost — it moves to the inner exception.
  `ZeeKayDaConfigurationException` and `ZeeKayDaConfigurationFailure` now document this as a contract
  binding anyone who throws them, including a custom `IClientRepository`, `IScopeRepository`, or
...
- **The discovery document advertises `claims_supported`** (#716). `OpenIdConfigurationDocument`
  ...
  It is derived, never configured: the ID Token and UserInfo claims declared by every discoverable
  scope in the `IScopeRepository`, plus the ten protocol claims an ID token carries (`iss`, `sub`,
  `aud`, `iat`, `exp`, `auth_time` and `at_hash`, and `nonce`, `acr` and `amr` when the request or
  the grant has them). A
...
- **The token endpoint: the authorization code grant with PKCE** (#71)
  ...
  Lifetimes are new options: `TokenEndpoint.AccessTokenLifetime` (ten minutes) and
...
- **A client registration that cannot be read is logged once, not on every request** (#698). A
  ...
  `DbContext`, say — never reached the verdict cache, so it wrote a `Critical` log entry per
...
- **BREAKING: Azure Key Vault cached signing is now an `ISigningKeySource`, downloading private material for exactly one certificate version** (#520)
  ...
  `AzureKeyVaultCachedSigningKeySource` and a `StaticSigningKeyRing` over it. The registration call
  is unchanged — one Key Vault certificate, one algorithm, one credential — and the vault is now
  read exactly once, at startup: rotation is picked up by restarting the host.

  The version-to-slot derivation is **shared with the remote provider** (#519), moved into
  `KeyVaultVersionSelector.SelectVersions` and identical for both: the newest enabled version
  inside its own validity window that has existed for `PreActivationDelay` (default one day,
  first-ever version exempt) signs; every newer enabled version is published as staged; up to
  `PreviousVersionsToPublish` (default 1) older enabled versions stay published; disabling a
  version in the vault removes it from every slot. `AzureKeyVaultCachedSigningOptions` loses its
  `KeySourceOptions` base (`RefreshInterval`/`PublicationLead` gone) and gains those two properties.

  What is specific to the cached provider tightened rather than changed:

  - **Private material is downloaded for exactly one version — the signing one.** `ReadAsync`
    reads every published version, the signing one included, as public `Cer` material only, so
