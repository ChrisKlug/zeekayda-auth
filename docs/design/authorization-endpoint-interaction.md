...
{
    // Null when nothing is parked for the interaction — absent, expired or misbound — recoverable.
    ...
    // subject, the provider's claims — plus these. A sub or NameIdentifier claim is refused;
    // zkd:* is stripped. No amr. Refuses when nothing is parked. zkd_i-bound on SignInAsync's terms.
    Task SignInAsync(params Claim[] claims);

    // Terminal. Links to a local account: the host's own principal, subject included, exactly as
    // ILoginInteraction.SignInAsync takes it. Refuses when nothing is parked; consumes it otherwise.
    Task SignInAsync(ClaimsPrincipal principal, params string[] authenticationMethods);
...
public sealed class PendingPrincipal                     // IProviderSignInInteraction.GetAsync, on the collect-more page
{
...
the session; the `ClaimsPrincipal` overload is for linking to a local account, where the host's
own principal, subject included, is what the session holds. Both consume the parked principal.
One service per host page: the login page's `ILoginInteraction` does not read the parked principal.
