...
**Analyzer enforcement:** a companion Roslyn analyzer (diagnostic ID to be assigned — see issue #230) emits a Warning-severity diagnostic on any out-of-assembly `IClientRepository` implementation that never references `IClientRegistrationValidator` anywhere in its type body (no constructor parameter, field, or method call of that type). In-assembly implementations (`ZeeKayDa.Auth` itself) are exempt, mirroring the `ZEEKAYDA0002` precedent. The check is a heuristic — it detects the absence of any reference, not that every write or load path calls the validator correctly — but this is sufficient as a defence-in-depth control: a mis-wired validator surfaces as `invalid_client` at runtime rather than opening a silent exploit window, so a heuristic build warning is the proportionate enforcement level.
