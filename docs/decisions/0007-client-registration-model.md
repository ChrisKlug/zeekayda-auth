...
**Startup validation:** every `TokenEndpoint.AuthMethodsSupported` value MUST be present in at least one registered `IClientAuthenticator.AuthenticationMethods`, **no method string MAY appear in more than one registered authenticator's `AuthenticationMethods`** (every method has exactly one owner; ambiguous ownership is a startup failure, not a runtime collision misdiagnosed as multi-mechanism), and every in-memory client's `AllowedTokenEndpointAuthMethods` value MUST be a subset of `TokenEndpoint.AuthMethodsSupported`. Any failure produces `ValidateOptionsResult.Fail(...)` at startup. Custom repositories own equivalent validation.
