...
`AddZeeKayDaAuth()` MUST register `Pbkdf2ClientSecretHasher` as the default `IClientSecretHasher` implementation automatically, so that the easy path is the secure path. Operators who need a different hasher can replace the registration by calling `AddSecretsHasher<THasher>(isDefault: true)` after `AddZeeKayDaAuth()`.
...
**Startup validation:** every `TokenEndpoint.AuthMethodsSupported` value except `TokenEndpointAuthMethods.None` MUST be present in exactly one registered `IClientAuthenticator.AuthenticationMethods`; `TokenEndpointAuthMethods.None` MUST NOT appear in any registered authenticator's `AuthenticationMethods` (it is reserved to the composite fallback); and every in-memory client's `AllowedTokenEndpointAuthMethods` value MUST be a subset of `TokenEndpoint.AuthMethodsSupported` (this per-client subset check is enforced by `IClientRegistrationValidator` §6.1, called by `InMemoryClientRepository` at construction time via `ClientRepositoryStartupActivator` — the `AuthenticatorCoverageValidator`'s remit is limited to authenticator-to-server-allowlist coverage and the reserved `none` constraint). Any failure produces `ValidateOptionsResult.Fail(...)` at startup. Custom repositories own equivalent validation.

**Layering:** because `IClientAuthenticator` lives in `ZeeKayDa.Auth.AspNetCore` (§9) but `TokenEndpointOptions` lives in `ZeeKayDa.Auth` (Core), the coverage validator (`AuthenticatorCoverageValidator`) necessarily lives in AspNetCore and cannot be a Core `IValidateOptions<TokenEndpointOptions>` implementation. Implementation issues shipping this validator MUST place it in the AspNetCore package.
...
**Defence in depth:** the runtime defences in §4 and §7 partition into two classes, and the partitioning matters. Defences that have a runtime twin — token-endpoint dispatch hardening (the §4 dispatch rules) and the §7 enumeration mitigations (timing padding, non-disclosure) — remain unconditional at request time: the token endpoints fail closed for unknown or unauthorized clients regardless of whether any repository called the validator. For these, a validator gap in a custom repository surfaces as runtime `invalid_client` / `invalid_request` rejections — incorrect operator-visible behaviour, not an exploit window. Registration-time invariants enforced *only* by `IClientRegistrationValidator`, however, have **no** runtime twin: the PBKDF2 iteration floor, the two-active-shared-secret cap, the redirect URI rules' registration-time enforcement, the `IsPublic` consistency table, and the per-client allowlist subset checks are applied at write/load time and are not re-checked on the request path. A custom `IClientRepository` that never resolves and calls the validator can therefore persist or serve registrations that violate these constraints, with no runtime catch — a security-relevant gap, not merely a behavioural one. This is why §6.1 makes the validator call a hard obligation on every custom repository, and why the `ZEEKAYDA0003` analyzer flags implementations that never reference it.

**Analyzer enforcement:** a companion Roslyn analyzer (`ZEEKAYDA0003`) emits a Warning-severity diagnostic on any out-of-assembly `IClientRepository` implementation that never references `IClientRegistrationValidator` anywhere in its type body (no constructor parameter, field, property, local variable, method call, or type reference of that type). In-assembly implementations (`ZeeKayDa.Auth` itself) are exempt, mirroring the `ZEEKAYDA0002` precedent. The check is a heuristic — it detects the absence of any reference, not that every write or load path calls the validator correctly — but this is sufficient as a defence-in-depth control: a mis-wired validator surfaces as `invalid_client` at runtime rather than opening a silent exploit window, so a heuristic build warning is the proportionate enforcement level.
...
> now enforced by `check_log_hygiene.cs`, which reads MSBuild's and Roslyn's own resolution of
> effective diagnostic severity per source file, and treats any project-wide downgrade of the
> enforcing analyzers (`ZEEKAYDA0001`/`ZEEKAYDA0002`) as a hard failure with no justification
> escape hatch. See [Analyzer rules](../reference/analyzer-rules.md) for the current suppression
> model.
...
| `IClientSecretFactory` | `ZeeKayDa.Auth` |
...
- **2026-06-13 (ADR 0006/0007 layering ruling — composite hasher misconfiguration)** — Architecture review finding AA-M10 identified that `CompositeClientSecretHasher.ResolveDefault` was throwing `InvalidOperationException` for misconfiguration conditions (no hashers registered; multiple hashers with no default; default type not found in hasher list) and that ADR 0006 and ADR 0007 had conflicting guidance on which layer governs detection of these errors. **Ruling:** ADR 0007 §3.5 (`ClientSecretHasherOptionsValidator`) is the **primary detection layer** — these conditions are caught at startup via `ValidateOptionsResult.Fail(...)` when `ValidateOnStart()` is used. `CompositeClientSecretHasher.ResolveDefault` is a **defensive fallback layer** that fires if the startup validator was not enabled or was bypassed; at that layer the correct exception type is `ZeeKayDaConfigurationException` (ADR 0006 §2) — specifically, the three stable error codes are `configuration.hashers.none_registered`, `configuration.hashers.no_default`, and `configuration.hashers.default_type_not_found`. Both layers are intentional and complementary; they are not alternatives. ADR 0006 §2 governs the exception type; ADR 0007 §3.5 governs the primary detection mechanism. This is the same two-layer pattern documented in ADR 0006 §2 for `EndpointRouteHelper.GetIssuerUri`. Resolves #158.
...
- **2026-06-17 (§3.5 default hasher registration)** — Amend §3.5 to require that `AddZeeKayDaAuth()` registers `Pbkdf2ClientSecretHasher` as the default `IClientSecretHasher` automatically, so that the easy path is the secure path. Operators who need a different hasher call `AddSecretsHasher<THasher>(isDefault: true)` after `AddZeeKayDaAuth()` to replace the registration.
- **2026-06-19 (`IClientSecretFactory` public injectable seam)** — Add `IClientSecretFactory` as a public interface in `ZeeKayDa.Auth.Clients` (§9 package ownership table updated). `CompositeClientSecretHasher` implements the interface; `AddZeeKayDaAuth` registers an alias singleton `services.TryAddSingleton<IClientSecretFactory>(sp => sp.GetRequiredService<CompositeClientSecretHasher>())`. The prior state left custom `IClientRepository` authors without an injectable seam for hashing secrets at runtime — they had to take an undocumented dependency on `CompositeClientSecretHasher` (internal) or `IClientSecretHasher` (the full hasher contract, which carries the full `CanHandle`/`Verify`/`GetRegistrationFailures` surface not needed for creation). `IClientSecretFactory` exposes the single `Create(string plaintext)` operation, clearly scoped to administrative use only (CPU-intensive; MUST NOT be called on a hot request path; calling endpoint must be rate-limited and authenticated). Closes #135.
- **2026-06-19 (ADR 0007 amendment — §4/§6.1 placement clarification and analyzer record)** — Two additive clarifications following the post-implementation review of #146. (1) §4 startup-validation paragraph gains a parenthetical on the per-client `AllowedTokenEndpointAuthMethods` subset check, making explicit that this check is `IClientRegistrationValidator` (§6.1)'s responsibility (run at startup via `ClientRepositoryStartupActivator`) and that `AuthenticatorCoverageValidator`'s remit is limited to authenticator-to-server-allowlist coverage and the reserved `none` constraint. (2) §6.1 gains a note recording the companion Roslyn analyzer (issue #230) — Warning severity, in-assembly exemption, heuristic nature (references, not proves-called), and defence-in-depth rationale. No decision text altered. Closes #231.
- **2026-08-18 (ADR 0007 amendment — §7 log-hygiene enforcement mechanism)** — §7's log-never list (`client_secret`, `code_verifier`, etc.) is unchanged; the note added records that its *enforcement* moved from `check_log_hygiene.sh` (suppression-syntax enumeration, shown not to converge — 12 additional bypass vectors found) to `check_log_hygiene.cs` (MSBuild/Roslyn effective-severity resolution, with no project-wide suppression escape hatch for the enforcing analyzers). No decision text altered; see [Analyzer rules](../reference/analyzer-rules.md) for the current model. Closes #459.
