...
| Clock skew tolerance | `AuthorizationServerOptions.ClockSkewTolerance` | `5 s` | Deployment property, not a store implementation detail. Applied as a grace window on `ExpiresAt` liveness checks in the distributed cache store (`entry.ExpiresAt + ClockSkewTolerance > now`). Does not affect the in-memory store (single-instance — no inter-node skew possible) or tombstone TTL (dominated by `RefreshTokenLifetime`). Default is intentionally small; see §"Security Considerations — Clock skew tolerance". |
...
The tolerance applies only to the distributed cache store implementation's `ExpiresAt`
liveness check. It does not affect the in-memory store (single-instance deployment — no
inter-node skew possible) and does not affect tombstone TTL (the `RefreshTokenLifetime`
value dominates any reasonable skew by orders of magnitude).
