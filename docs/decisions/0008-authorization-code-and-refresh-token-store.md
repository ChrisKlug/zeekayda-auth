...
- **2026-07-11 — `AllowInMemoryStoresOutsideDevelopment` moves off `AuthorizationServerOptions` onto the in-memory store registration methods** — The escape hatch that permits in-memory stores outside `Development` (fail-closed by default: outside `Development`, startup throws `ZeeKayDaConfigurationException` unless it is opted in) previously lived as a `bool` on the shared `AuthorizationServerOptions` root. It moves onto the three in-memory registration methods themselves as a `bool allowOutsideDevelopment = false` parameter: `AddInMemoryStores(bool allowOutsideDevelopment = false)`, `AddInMemoryAuthorizationCodeStore(bool allowOutsideDevelopment = false)`, and `AddInMemoryRefreshTokenStore(bool allowOutsideDevelopment = false)`. Each method gates independently on its own parameter — mixing granular calls with different values is allowed and each is enforced independently. **No new options type is introduced**: per this ADR's own precedent (the 2026-06-21 removal of the empty `DistributedCacheTokenStoreOptions`), a single flag does not justify an options class; a method parameter is the right shape. The flag is meaningless unless one of these methods was called, so it belongs on those methods rather than on the shared root — the same reasoning that keeps it out of bindable configuration. **Invariants unchanged:** outside `Development`, startup still fails unless the relevant method's `allowOutsideDevelopment` is `true`; the mandatory `InMemoryStoreWarningService` startup warning (§5) still fires whenever an in-memory store is registered outside `Development` with the flag set. This is one decision made together with ADR 0011's equivalent move of `AllowedDevelopmentJwtSigningKeysEnvironments` off the root onto the development-signing-key provider options — both reverse PR #333's root-hoisting precedent (see the ADR 0002 grouping-rule scope clarification). Design: issue #337; implementation: issue #339.
