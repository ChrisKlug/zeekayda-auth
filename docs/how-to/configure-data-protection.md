...
- `zkd.session` — SSO session cookie, shared across all authentication methods. This is the
  longest-lived of the cookies: its lifetime is tied to the SSO session rather than to a
...
- `zkd.interaction.<id>` — not a scheme: one small cookie per authorization request, holding a
  ...
  naming the client, kept for a day after the request ends. The request
