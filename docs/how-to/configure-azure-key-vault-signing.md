...
> ⚠️ **Warning:** `RefreshInterval` here also governs how often the provider re-downloads private key material for every in-window certificate version — this is more sensitive traffic than remote signing's refresh, which only ever fetches public key information. Don't set `RefreshInterval` too low purely out of habit; balance rotation responsiveness against how often you want private key material moving over the network and being re-materialized in process memory.

At startup, an informational log line records that the private key has been downloaded and is cached in process memory, including the configured certificate identifier. This is logged at `Information`, not `Warning` — it is a deliberate architectural consequence of choosing this provider, not a misconfiguration.
