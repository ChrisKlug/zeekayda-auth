...
> ⚠️ **Warning:** Every `RefreshInterval` tick, the provider re-lists Key Vault's certificate versions and re-fetches **public** key material for every enabled version — there is no change-detection short-circuit, so this happens on every tick regardless of whether anything actually changed. The active certificate's **private** key (via the certificate's linked secret) is only re-downloaded when the active version itself changes, never on a tick where the same version is still active. Even so, an active-version rotation is more sensitive traffic than remote signing's refresh, which only ever fetches public key information for any version. Don't set `RefreshInterval` too low purely out of habit; balance rotation responsiveness (including how quickly an emergency revocation is noticed — see the warning under [Option 1](#option-1--remote-signing)) against how often the active certificate's private key material moves over the network and is re-materialized in process memory.

An informational log line, emitted once at startup, records that the currently active certificate's private key will be downloaded and cached in process memory on first use, including the configured certificate identifier — the download itself is lazy and happens the first time this provider is asked to sign, not necessarily at startup. This is logged at `Information`, not `Warning` — it is a deliberate architectural consequence of choosing this provider, not a misconfiguration.
...
for the full explanation of `max(CreatedOn + PublicationLead, NotBefore)`.
...
least `PublicationLead`. Set Key Vault's own rotation trigger with enough lead time
