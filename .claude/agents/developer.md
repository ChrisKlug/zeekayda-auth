...
- **Never `new Uri(relative)` — in tests too.** On macOS and Linux a string starting with `/` parses as an absolute *file* URI, so `new Uri("/login?zkd_i=abc").Query` is empty: the query is silently lost. A redirect's `Location` is often relative, which is exactly how a smoke test once dropped `zkd_i` and failed two requests later. Resolve against a real base (`new Uri(baseUri, location)`), or slice the query from the string.
- **Copying an existing helper does not import its exemptions.** Several review findings and CodeQL hits have come from lifting a helper out of a neighbouring file that predates a standard. Bring it up to the current standards, or leave it where it is.
- **Before calling an existing primitive, read how its other callers guard it.** A caller's catch list or pre-check often records a platform quirk the primitive does not handle itself: `SigningSelfTest.TryVerify` catches the `ArgumentException` some platforms throw from `SigningAlgorithms.Verify` on a wrong-length signature, and a new caller that skipped that read let it escape on request input.
- **Run a completeness guard against the omission it claims to catch, before trusting it.** A reflection test naming every member, or a theory mutating one at a time, can pass while the thing it guards is broken. `ClientRegistrationSnapshot`'s first guard mutated the source *after* the copy was taken, so a member the copy forgot came out as `false` — the same value the source held before the mutation — and deleting a copy line left all 28 tests green. Delete the line, watch it fail, put it back — commit first and restore with `git checkout -- <file>`, or `touch` a file restored from a copy. `mv` from a `cp` backup keeps the backup's older timestamp, so MSBuild skips the rebuild and every later run tests the mutated binary. The guard that worked compares a fully-populated fixture against its own copy, with a second test proving no member sits at a default an omission could match.
- **Changing a documented behaviour sweeps the docs for every way it is phrased, not just its identifier.** A changed log level, error code or default is described in prose far more often than by name: search `docs/` and `samples/` for the old value (`LogLevel.Warning`), for the words that describe it ("startup warning", "emits a warning"), and for the code. Searching only for a warning's code once found half the stale passages; the PR reviewer found the rest.
